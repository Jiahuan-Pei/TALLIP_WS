...
from Single.linearcombination_en import *
import codecs
import numpy
import random
import math
...
class ExperimentRun:
    ...
    def __init__(self, hypers):
        ...
        """
        pretrained_vectors_filepath = r"%s/GoogleNews-vectors-negative300.bin" % macro.DICT_DIR
        # vocabulary_filepath = r"D:\MyData\NLPCC_DATA\corpus\vocab.txt"
        # input
        self.eval_fname = 'SimLex999.txt'  # 评价文件名 #
        # self.eval_fname = 'wordsim353.txt'  # 评价文件名 #
        ...
        # 同义和反义词典(固定的)
        antonym_list = [r'%s/ppdb_antonyms.txt' % macro.DICT_DIR]
        synonym_list = [r'%s/ppdb_synonyms.txt' % macro.DICT_DIR]
        # 根据single_sim们计算的词对（可变的）
        antonym_value_list = [r'%s/antonyms_value.txt' % macro.DICT_DIR]
        synonym_value_list = [r'%s/synonyms_value.txt' % macro.DICT_DIR]
        df = single_sims([(macro.CORPUS_DIR, self.eval_fname)], '%s_single_sims_en' % self.eval_fname.split('.')[0])
