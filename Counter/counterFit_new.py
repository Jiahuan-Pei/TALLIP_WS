...
"""
    ...
    @function: 首先在countFit_dev里面调参，确定参数后在test函数里面固定
"""
...
class ExperimentRun:
    ...
    def __init__(self, hypers):
        ...
        self.eval_fname = 'MC30.txt'  #
        # self.eval_fname = 'SemEval50.txt'
        ...
        df = single_sims([(macro.CORPUS_DIR, self.eval_fname)], '%s_single_sims' % self.eval_fname.split('.')[0])
...
def counter_fit(current_experiment):
    ...
    _, _, _, sims, _, _ = analysis(word_vectors, current_experiment.read2wordlist, 'improved')
    _, _, _, sims2, _, _ = analysis(word_vectors2, current_experiment.read2wordlist, 'basic')
    ...
    plt.plot(times, spearmans, 'k-', label='improved counter-fitting', linewidth=1.5)
    plt.plot(times, spearmans2, 'k--', label='basic counter-fitting', linewidth=1)
...
def analysis(word_vectoers, read2wordlist, mode='old'):
    '''
...
def run_experiment(hypers):
    ...
    _, _, _, _, pp, rr = analysis(current_experiment.pretrained_word_vectors, current_experiment.read2wordlist, mode='old')
    transformed_word_vectors = counter_fit(current_experiment)

    #
    print "\nSpearman's rho coefficient of the counter-fitted vectors is:"
    idl, w1l, w2l, sims, p, r = analysis(transformed_word_vectors, current_experiment.read2wordlist, mode='new')
    ...
    print_word_vectors(current_experiment.pretrained_word_vectors, macro.RESULTS_DIR+r'/%s_org_vectors.txt' % current_experiment.eval_fname.split('.')[0])
    print_word_vectors(transformed_word_vectors, macro.RESULTS_DIR+r"/%s_counter_fitted_vectors.txt" % current_experiment.eval_fname.split('.')[0])
    return hypers_str


def test():
    # k1, k2, k3, k4, k5 = 0.1, 0.1, 0.7, 0.1, 0.0          # 默认
    # delta, gamma, rho, theta, eta = 0.5, 0.0, 0.2, 0.0, 1

    # k1, k2, k3, k4, k5 = 0.18, 0.14, 0.1, 0.02, 0.0       # g1
    # delta, gamma, rho, theta, eta = 0.5, 0.0, 0.2, 0.28, 1

    # k1, k2, k3, k4, k5 = 0.01, 0.17, 0.1, 0.03, 0.0       # g2
    # delta, gamma, rho, theta, eta = 0.5, 0.0, 0.2, 0.18, 1

    # k1, k2, k3, k4, k5 = 0.16, 0.17, 0.1, 0.03, 0.0       # g3
    # delta, gamma, rho, theta, eta = 0.5, 0.0, 0.2, 0.25, 1

    # k1, k2, k3, k4, k5 = 0.01, 0.12, 0.1, 0.08, 0.0       # g4
    # delta, gamma, rho, theta, eta = 0.5, 0.0, 0.2, 0.23, 1

    # k1, k2, k3, k4, k5 = 0.02, 0.16, 0.1, 0.09, 0.0       # g5
    # delta, gamma, rho, theta, eta = 0.5, 0.0, 0.2, 0.04, 1

    # k1, k2, k3, k4, k5 = 0.076, 0.152, 0.1, 0.05, 0.0     # avg1
    # delta, gamma, rho, theta, eta = 0.5, 0.0, 0.2, 0.196, 1

    # k1, k2, k3, k4, k5 = 0.01, 0.15, 0.1, 0.03, 0.0       # avg2
    # delta, gamma, rho, theta, eta = 0.5, 0.0, 0.2, 0.38, 1

    k1, k2, k3, k4, k5 = 0.02, 0.08, 0.76, 0.14, 0.0     # others
...
if __name__ == '__main__':
    test()
    # w2v_model = KeyedVectors.load_word2vec_format(macro.MODELS_DIR + '/fml_org_bdnews_xieso_w2v.bin', binary=True)
    # print w2v_model.similarity(u'电脑',u'计算机')
